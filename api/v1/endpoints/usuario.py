...
# identifica que determinado endpoints precisa de autendicacao
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    if not usuario:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, delattr='Dados de acesso incorretos.')
    
    return JSONResponse(content={'access_token': _criar_token_acesso(sub=usuario.id), "token_type": "bearer"}, status_code=status.HTTP_200_OK)
